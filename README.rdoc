...
changes at the beginning then transposes all notes up one octave.
...
Things get a bit tricker if you want to split the incoming MIDI up somehow
so that you can process it with one list of filters for one destination and
another list for another destination.

    (define tp-flow-1
       (list "Tommy's Place Flow 1"
             tp-pre-proc ; defined elsewhere
